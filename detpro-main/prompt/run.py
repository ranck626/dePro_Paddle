...
from classname import *    # 只能说星号太阴险了
import coop_mini
...
def load_data_new(data_path : str, neg_thr = None, remap = False):    # 为啥要申明str？  str for  string
    features, labels, ious = torch.load(data_path)# 注意是复数,data_path不就是路径吗  理解为加载数据，然后赋值给ious？
    
    if neg_thr == None:  # 27~31 不懂，是bool值吗？，如果是的话，为什么可以labels【neg】
        neg = (labels < 0)  # 定义了一个 元组嘛？
    else:
        neg = (ious < neg_thr)
        labels[neg] = -1

    base = labels.new_zeros(labels.shape).bool()   #为啥一样啊
    ...
    if remap:
        mapping = torch.Tensor(lvis_base_label_ids).long()   # 整个复制过来，然后摸了摸有多长
        base_labels = (labels[base].view(-1,1)==mapping).int().argmax(dim=1)  # view（-1,1）x * 1 维度 因地制宜
        base_data = features[base], base_labels, ious[base]
        neg_data = features[neg], (labels).new_ones(labels[neg].shape)*866, ious[neg]
    else:
        baase_dat = features[base], labels[base], ious[base]
        neg_data = features[neg], labels[neg], ious[neg]
    novel_data = features[novel], labels[novel], ious[novel]    #为啥能这样赋值啊
    
    return base_data, novel_data, neg_data

def data_iou_filter(data, lb, ub):
    features, labels, ious = data
    valid = torch.logical_and(lb <= ious, ious < ub)  # and ,两个都得满足
    return features[valid], labels[valid], ious[valid]

def get_freq(data):#标签相同的data有多少
    freq = [0] * 1204   #????
...
def load_ens_embedding(name_list, norm = False,weight=None):
    ...
    if weight is not None:
        emb = [x*w / x.norm(dim=-1, keepdim = True) for x,w in zip(emb,weight)] # zip 配对er
    else:
